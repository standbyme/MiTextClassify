...
object main extends App {
  ...
  def topic_handler(word_with_probability_seq: Seq[WordWithProbability])(topic: Topic): (Topic, SimEmbeddingScore) = {
    println(topic)

    val scores = word_with_probability_seq.flatMap(wordSimEmbeddingAdapter(_, topic)).map(word_with_probability_and_score_handler)
    val average = scores.sum / scores.length
    println(scores.length)
    (topic, average)
  }

  def word_with_probability_seq_handler(word_with_probability_seq: Seq[WordWithProbability]): Option[Topic] = {
    val temp = topics.map(topic_handler(word_with_probability_seq)).sortBy(_._2)
  ...
  val word_with_probability_seq = Seq(
    ("医药",0.34692),
    ("药店",0.0579149),
    ("药品",0.0359799),
    ("药房",0.0339464),
  ...
  println(word_with_probability_seq_handler(word_with_probability_seq))
}
